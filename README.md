...
## 포함 내용
- `Runtime/` — 핸드포즈 적용(`HandPoseApplier`·`HandPoseDirectApplier`), 컨트롤러 그립 포즈(`ControllerGrabHandPose`·`ControllerGrabHandVisibility`), 스냅샷(`HandPoseData`·`HandPoseSnapshotRoot`·`HandPoseSnapshotValidator`), `Hand`·`Cube`·`SphereGizmoDrawer` 런타임 스크립트 + asmdef
...
## 데모 씬 실행 — 왼쪽 컨트롤러로 박스 잡기

준비(앞 단계 완료): XR 의존성 설치 → 필수 샘플(Starter Assets / HandVisualizer) Import → 프로젝트 설정 → Project Validation 통과.

1. **Demo 샘플 Import**: Package Manager에서 **VR Base Package ▸ Samples ▸ Demo ▸ Import**
2. 임포트된 데모 씬 열기:
   `Assets/Samples/VR Base Package/1.0.0/Demo/Test_1.unity`
3. Hierarchy 구성 확인:
   ```
   Test_1
   ├─ Directional Light
   ...
   │     └─ Left Controller
   │        ├─ Visuals
   │        ├─ Left_NearFarInteractor   ← 잡기용 인터랙터
   │        └─ Hand Visualizer
   └─ Cube                              ← 잡을 박스(그랩 대상)
   ```
4. **Play** 진입(헤드셋 연결 또는 Quest Link/무선). **왼손 컨트롤러**를 `Cube`에 가져가 **Grip 버튼**으로 잡으면, 핸드포즈가 적용되어 손이 박스 그립 모양으로 잡힙니다.

> 손이 그립 포즈로 안 잡히면: `Cube`에 그립 스크립트 + `HandPoseData`가 할당돼 있는지, `Left_NearFarInteractor`가 활성인지, OpenXR에 **Oculus Touch Controller Profile** + **Hand 기능**이 켜져 있는지 확인하세요.

## 쓰는 법
- 본인 XR 리그(컨트롤러 손)에 `XRHand_L/R` 컨트롤러를 Animator에 물리고, 잡을 오브젝트에 그립 스크립트(`ControllerGrabHandPose` 등) + `HandPoseData`를 할당하면 그립 포즈가 동작합니다.
